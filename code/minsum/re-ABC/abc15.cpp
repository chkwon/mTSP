...
// Name        : BLS_TSP.cpp
// Author      : pengfei he
// Version     : This code is used to implement BLS solving CTSP, colored traveling salesman problem
...
#include <algorithm>
using namespace std;
#define neb 100 // the number of employ bee
#define nob 150 // the number of onlooker bee
#define Max 100000000
#define Pbts 80
#define limit_scout 100
#define iter_max    2000
#define obj    1// i is objective 1 and 2 is objective 2
...
double ** Distance;
int step=1;
double P_method;// three parameters
double Pmin;
double Pmax;
int graph_type;
solution best_food;// to keep the best food
// all parameters which need to be adjusted should be added in basic.h.
solution food [neb];//in this code, the number of employ bee is the number of food
solution new_food;
clock_t start;
int optimal_time;
string aa;
...
//	char aa[1000];
	...
	//int num;
	while (1){
		open_file>>bb;open_file>>v_num;//open_file>>ex;
		break;
	}
...
void neighbor(int n){//this function is used to construct a new solution for food[n] and update it
// the main parameters are food[n] and new_food.
	// the first step is to initial new_food
	vector<int>unassign;// this vector is used to keep all nodes deleted from new_food
	extract_nodes(unassign,n);
/* 	for (int i=0;i<=v_num-1;i++){
		for (int j=0;j<=num_v;j++)
			cout<<new_food.sol_r[i][j]<<" ";
		cout<<endl;
	} */
	random_shuffle(unassign.begin(), unassign.end());// random all nodes in unassign
	// insert all nodes.
	insert_fun(unassign);
	if (food[n].fit>new_food.fit){
		food[n].no_impro=0;
		food[n].fit=new_food.fit;
		for (int i=0;i<=v_num-1;i++)
			for (int j=0; j<=num_v;j++){
				food[n].sol[i][j]=new_food.sol[i][j];
				food[n].sol_r[i][j]=new_food.sol_r[i][j];
			}
	}
	else food[n].no_impro++;
}
int bts (int c1, int c2){
	int re;
	if (rand()%100<Pbts){
		if (food[c1].fit > food[c2].fit) re=c2;
		else	re=c1;
	}
	else{
		if (food[c1].fit > food[c2].fit)	re=c1;
		else	re=c2;
	}
	return re;
}
void ABC(){
	...
	while(1){
		...
		for (int i=0;i<=nob-1;i++)	{
			int c_1,c_2;
			while(1){
				c_1=rand()%neb;c_2=rand()%neb;
				if (c_1!=c_2)
					break;
			}
			p=bts(c_1,c_2);// this function is used to find the probability which is used to select which food is onlook bee.
			neighbor(p);
		}
		determine_best(best_food.fit);
//		cout<<step<<"    "<<best_food.fit<<endl;
		for (int i=0;i<=neb-1;i++)	{
			if (food[i].no_impro>limit_scout){
				construct(i);food[i].no_impro=0;
			}
		}
		step++;
...
//		if(ceil((clock() - start)/static_cast<double>(CLOCKS_PER_SEC))>=time_length)
		//	break;
 		double tem;int pointer;double delta=0;
		for (int i=0;i<=v_num-1;i++){
			pointer=0;tem=0;
			while (1){
				tem+=Distance[pointer][best_food.sol[i][pointer]];
		...
		cout<<step<<"   "<<delta<<"    "<<best_food.fit<<endl;
	//	cout<<tem<<endl;
